...
object IPApp {
  ...
  val IMAGE_CATEGORIES = List("basketball","boxing","cricket","swimming","tennis")
  ...
  def testImageClassification(sc: SparkContext) = {
    ...
    val decisiontreemodel = DecisionTreeModel.load(sc, IPSettings.DECISION_TREE_PATH)
    //println(decisiontreemodel.labels.mkString(" "))

    val p = decisiontreemodel.predict(histogram)
